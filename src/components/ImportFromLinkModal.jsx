      ...
      <div className="import-link-modal" role="dialog" aria-modal="true" aria-label="Queue website for discovery">
        <div className="modal-head">
          <div>
            ...
            <h2>Queue link discovery</h2>
            <span>Submit a public URL for background discovery.</span>
          </div>
          <button type="button" className="icon-button refined" aria-label="Close import" onClick={onClose}><X size={18} /></button>
        </div>
        <div className="hybrid-discovery-banner" role="status">
          <strong>Hybrid discovery mode</strong>
          <span>Manual entries are saved instantly. Automatic discovery requests are queued and processed by GitHub Actions so the app can stay free. Results may take a few minutes to appear.</span>
        </div>
        ...
        <p className="discovery-status-note"><strong>Discovery status:</strong> "Queued" = waiting for GitHub Actions. "Processing" = discovery workflow is running. "Completed" = results are available. "Failed" = discovery could not complete.</p>
